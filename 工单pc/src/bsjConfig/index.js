...
class BSJ_CONFIG{
  ...
  update(){ //同步更新，获取用户数据更新congfig
    this.init();
  }
  updateSync(){ //异步更新，暂未有需要

  }
  getTrackMapImg({terminalType}){ //获取轨迹回放播放车辆的图标
    let km_06mConfig=this.data.module.km_06m;
    if(km_06mConfig.mapImg&&km_06mConfig.terminalTypeList.indexOf(terminalType.toLocaleUpperCase())!==-1){
      return require("@/bsjConfig/img/km-06m/map_online.png");
    }else{
        return require("@/assets/img/map/travel.png");
    }
  }
  getTrackMapAngle({terminalType,angle}){ //获取轨迹回放播放车辆旋转角度
    let km_06mConfig=this.data.module.km_06m;
    if(!km_06mConfig.mapRotate&&km_06mConfig.terminalTypeList.indexOf(terminalType.toLocaleUpperCase())!==-1){
