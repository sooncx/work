<template>
  <div :class="['selectVeh',!checked?'hide':'']">
    <div class="input-box">
      <el-autocomplete
        :disabled="!checked"
        class="inline-input"
    ...
    <transition
      name="bsj-zoom-in-left"
      @after-enter="showTreeBefor"
    >
      <section
        class="tree-chunk"
      ...
      >
        ...
        >
          ...
          <div class="col-vehList">
            <div
              class="error-box"
              v-show="!isVehLoading&&vheList.length===0"
            >
              <div
                v-if="selectGroupId"
                class="none-car"
...
export default {
  ...
  methods: {
    ...
    handleSelect(item) {
      this.$emit("getValue", this.displayInputVal);
      //输入框自动完成选择回调
      if (item["_type"] === "group") {
