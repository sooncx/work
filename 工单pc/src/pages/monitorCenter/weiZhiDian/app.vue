...
export default {
  ...
  computed:{
    displayData() {
      ...
      var isKong=this.weiZhiDianTotalData!==undefined&&this.weiZhiDianTotalData!==null&&this.weiZhiDianTotalData!=='';
      if(isKong){
  ...
  methods: {
    ...
    delWeiZhiDian(item,index){
      this.$confirm('是否删除该为位置点?', '提示', {
          ...
          this.$http({
            url:this.HOST+"/http/SafetyZone/DeletePoint.json?type="+item.type+"&id="+item.id
            }).then(rst=>{
              if(rst.flag===1){
                this.$message({
                  type: 'success',
