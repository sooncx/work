...
class Address {
  ...
  getGeoItem({itemParams,itemUserCancelToken}){
  ...
  getGeo({params,userCancelToken}){  //批量获取地理位置 每50条拆分一次 
    /*
    *params:{
  ...
  getGeoProcess({params,processCall}){// 批量获取地理位置 每50条拆分一次 和 getGeo的区别是50条拆分一次后也会分次返回给外部，用于数据大于1w的时候地址转换过慢的
    if(!params||!params.data.posList){
      console.error("params.posList不能为空")
      return false;
    };
