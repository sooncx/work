      ...
      <div class="tab-header">
        <div class="col-left">
          <div class="tabTitle-box fl">
            <div
              class="tabTitle-item"
              :class="{active:avtiveTab===0}"
              @click="changeAvtiveTab(0)"
            >
              全部({{total}})
            </div>
            ...
            <div
              class="tabTitle-item"
              :class="{active:avtiveTab===1}"
              @click="changeAvtiveTab(1)"
            >
              在库({{inTotal}})
            </div>
            <div
              class="tabTitle-item"
              :class="{active:avtiveTab===2}"
              @click="changeAvtiveTab(2)"
            >
              出库({{outTotal}})
            </div>
          </div>
          <div class="tabTitle-box fl">
            <div
              class="tabTitle-item"
              :class="{active:avtiveTab===3}"
              @click="changeAvtiveTab(3)"
            >
              掉电({{missTotal}})
            </div>
          </div>
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span
                  class="status-mark"
                  :class="'status-mark-'+scope.row.statusCode"
  ...
  computed: {
    displayTableDataALL() {
      ...
      if (!this.searchVal) {
        ...
        result.forEach(item => {
          if (
            item.groupName.toLowerCase().indexOf(searchVal) !== -1 ||
            item.plate.toLowerCase().indexOf(searchVal) !== -1 ||
            item.frameNo.toLowerCase().indexOf(searchVal) !== -1
          ) {
            newResult.push(item);
          }
        });
    ...
    curDisplayTableData() {
      return this.displayTableDataALL.slice(
        this.page.size * (this.page.number - 1),
  ...
  methods: {
    getDiaodianListData() {
      ...
      this.$http({
        url:
          this.HOST +
          "/report/InOutStockReport/QueryInOutStockReport.json?status=3"
      }).then(rst => {
        if (rst.flag === 1) {
    ...
    doExport() {
      ...
      window.open("/http/ExcelExport/ExportInOutStock.json?" + stateTxt);
    }
  },
  watch: {
    searchVal() {
