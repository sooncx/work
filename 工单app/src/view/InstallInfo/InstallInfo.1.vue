<template>
  <div class="installInfo">
    ...
    <div class="mainbox">
      <div class="companyName">
        ...
        <div class="installHeader">
          ...
          <div class="installMain">
            <div v-for="(list,index) in deviceVos" :key="index">
              ...
              <div class="seiBei_left">
                ...
                <span class="lineStyle" :class="list.bgColor">{{list.isOnlineType}}</span>
      ...
      <div  class="main">
        <div class="main_contant">
          ...
          <div class="carPicStyle">
            <div  v-for="(lists,indexs) in carList_CS" :key="indexs">
              <img class="imgStyle" v-show="lists.isShowPic_CS" :src="lists.basestr_CS" @click="doSelectPhoto_CS(indexs)">
              <span class="main_right_upload" v-show="!lists.isShowPic_CS" @click="doSelectPhoto_CS(indexs)">
              <div class="iconfont icon-jia1"></div>
              </span>
              <div class="loadPic2" v-show="lists.isShowLoadPic_CS">加载中...</div>
      ...
      <div class="main" v-for="(lists,indexs) in deviceVos" :key="indexs">
        ...
        <div class="main_contant">
          ...
          <span class="main_right"><input type="text" class="placeholderStyle" v-model="installPositions[indexs]" placeholder="请输入安装位置"></span>
        </div>
        <div class="main_contant">
          <span class="main_left">相关照片</span>
          <img class="imgStyle" v-show="lists.isShowPic" :src="lists.basestr" @click="doSelectPhoto(lists.deviceId,$event,indexs)">
          <span class="main_right_upload" v-show="!lists.isShowPic" @click="doSelectPhoto(lists.deviceId,$event,indexs)">
            <div class="iconfont icon-jia1"></div>
          </span>
          <div class="loadPic" v-show="lists.isShowLoadPic">正在加载图片...</div>
        </div>
      </div>

     
    </div>
      <div class="footer" @click="upload_install()">
  ...
  methods: {
    ...
    onConfirm_CS(base64Img,deviceIndex) {
      ...
      this.$byApi.uploadPicture(param).then(res => {
        if (res.data.flag == 1) {
          ...
          // this.basestr_CS=basestr;
          // this.pics_CS=res.data.obj;
          // this.isShowPic_CS=true;
          // this.isShowLoadPic_CS=false;
        }
      });
    },
    ...
    getFromVehicleId(vehicleId, n) {
      ...
      this.$byApi.getCarLocationInfo(param).then(res => {
        if (res.data.flag == 1) {
          this.$set(that.deviceVos[n], "isOnlineType", that.isOnlines(res.data.obj.Z,res.data.obj.B));
          this.$set(that.deviceVos[n], "bgColor", that.bgColor(res.data.obj.Z,res.data.obj.B));
        }
      }).catch(()=>{
    ...
    isOnlines(Z,B){
     ...
     statuShow(key) {
      switch (Number(key)) {
        ...
        case 2: ///////待安装
          this.timeType = "确认时间";
          this.timeData=JSON.parse(localStorage.getItem("GD_data")).confirmTime  
          break;
        case 3: ///////已安装
          this.timeType = "安装时间";
        ...
        case 4: ///////已完成
          this.timeType = "完成时间";
          this.timeData=JSON.parse(localStorage.getItem("GD_data")).finishTime  
          break;
        case 5: ///////已取消
          this.timeType = "取消时间";
          this.timeData=JSON.parse(localStorage.getItem("GD_data")).cancelTime  
          break;
      }
    },
    upload_install() {
      ...
      if(this.isRequest){
        ...
        this.$byApi.uploadWorkSheetInfo(param).then(res => {
        if (res.data.flag == 1) {
          this.isClick=false;
          this.isRequest=true;
          this.isShowTC = true;
