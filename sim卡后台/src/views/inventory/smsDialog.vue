...
export default {
  methods: {
    ...
    sendBatchSms() {
      ...
      // return;
      this.$API.sendBatchSms(params).then(res => {
        if (res.code !== 0) this.setNowSendMsgData(smsIndex, err.data, 2);
    ...
    getSendSmsStatus(smsIndex, contentId, iccids, createTime) {
      let timer = setInterval(() => {
        ...
        this.$API.getSendSmsStatus(params).then(res => {
          if (res.code !== 0) {
            clearInterval(timer);
    ...
    getDevReplyNewOne(cardNo, counts, createTime) {
      ...
      let timer = setInterval(() => {
        ...
        if (!this.visible) clearInterval(timer);
        this.$API.getDevReplyNewOne({ cardNo }).then(res => {
          if (res.code !== 0) clearInterval(timer);
    ...
    getDevReply(cardNo, counts) {
      ...
      let timer = setInterval(() => {
        ...
        if (num === counts) clearInterval(timer);
        if (!this.visible) clearInterval(timer);
        this.$API.getDevReply({ cardNo }).then(res => {
          if (res.code !== 0) clearInterval(timer);
          if (!res.data.length) return;
          clearInterval(timer);
